...
async function syncSingleFeed(feedId: string): Promise<{ added: number; total: number; error?: string }> {
...
export const syncRssFeed = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    return syncSingleFeed(data.feedId);
  });
...
export const syncAllRssFeeds = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    for (const f of feeds ?? []) {
      try {
        const r = await syncSingleFeed(f.id);
